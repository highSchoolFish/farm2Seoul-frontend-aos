...
import sim.farm2seoul.databinding.DailyAuctionCardBinding
import sim.farm2seoul.presentation.activity.DailyAuctionDetail
import java.text.DecimalFormat

class PagingDataRecyclerViewAdapter :
    PagingDataAdapter<sim.farm2seoul.data.model.RowItems, PagingDataRecyclerViewAdapter.ViewHolder>(ItemDiffCallback) {
    ...
    inner class ViewHolder(private val binding: DailyAuctionCardBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(item: sim.farm2seoul.data.model.RowItems?) {
            item?.let {
                when (item.GRADENAME) {
    ...
    object ItemDiffCallback : DiffUtil.ItemCallback<sim.farm2seoul.data.model.RowItems>() {
        //두 객체를 비교해 링크가 같은지 체크
        override fun areItemsTheSame(oldItem: sim.farm2seoul.data.model.RowItems, newItem: sim.farm2seoul.data.model.RowItems): Boolean {
            return oldItem.PUMNAME == newItem.PUMNAME && oldItem.GRADENAME == newItem.GRADENAME
        }

        //두 객체의 내용이 같은지 확인
        override fun areContentsTheSame(oldItem: sim.farm2seoul.data.model.RowItems, newItem: sim.farm2seoul.data.model.RowItems): Boolean {
